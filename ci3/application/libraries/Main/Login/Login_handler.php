...
class Login_handler
{
    ...
    private function check_user_address($account_id){
        if(!empty($account_id)&&intval($account_id)>0){
            ...
            if(!empty($b) && intval($b)>0 && $this->users_model->add_address_relation([
                'address_id'=>intval($b),
                'target_table'=>'user_account',
                'target_id'=>intval($account_id),
            ])) $this->CI->session->set_userdata('user_address_id',$b);
        }
    }
        ...
        intval(end($a)['id']) > 0){
            ...
            intval(end($a)['user_id']) > 0){
                ...
                if(!empty($user_info) && !empty(end($user_info))){
                    ...
                    $this->user->check_user_rule($account_id);
    ...
    public function register_user($arr){
        ...
        !empty($arr['name']) && !empty($arr['family'])){
            ...
            ($check_add=$this->users_model->select_account_where_mobile_id(intval($this->CI->session->userdata('mobile_id'))))!==false && !empty($check_add) && !empty(end($check_add)) && !empty(end($check_add)['id']) && intval(end($check_add)['id'])>0 && intval(end($check_add)['id'])===intval($this->CI->session->userdata('account_id'))){
    ...
    public function send_phone_login($data){
        if(!empty($data) && $this->security->validate_mobile_number($data) && $this->CI->session->has_userdata('phone_number') && !empty($this->CI->session->userdata('phone_number')) && $data===$this->CI->session->userdata('phone_number')){
            $this->CI->session->set_userdata('login_code',['code'=>rand(10000,99999),'phone'=>$data]);
