...
class Upload_handler
{
    ...
    private function add_retern_id($type,$filename,$url){
        ...
        return $this->media_model->add_return_id(['filename'=>$filename,'url'=>base_url('storage/'.$type.'s/'.$url),'type'=>$type,'user_sign'=>$sing,'created_at'=>date('Y-m-d H:i:s')]);
    }
    private function upload_image_handler($image,$url){
        ...
        if (!empty($image) && preg_match('/^data:image\/(\w+);base64,/', $image, $type)) {
            ...
            $id=$this->add_retern_id('image',$filename,$url. $filename);
            $result = [
                'id'   => $id,
    ...
    public function upload_single_image($data){
        if(!empty($data) && !empty($data['data'])){
            ...
            if(is_null($check)){
                $url=(!empty($data['url'])?$this->security->string_security_check($data['url']):'');
                $to_action=(!empty($data['toAction']) ? $this->security->string_security_check($data['toAction']) : '');
                $a=$this->upload_image_handler($data['data'],$url);
