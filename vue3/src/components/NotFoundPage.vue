<template>
  ...
  <div class="astronaut">
    ...
    <div class="astronaut__cord">
      <canvas ref="cord" width="500" height="500"></canvas>
    </div>
    <div class="astronaut__head">
      <canvas ref="visor" width="60" height="60"></canvas>
...
<script setup>
  ...
  function animate(ctx) {
    function loop() {
      ...
      requestAnimationFrame(loop); // ✅ ctx اینجا داخل closure هست
    }
    loop();
  }
  ...
  onMounted(() => {
    nextTick(() => {
      if (visor.value) {
        const visorCtx = visor.value.getContext('2d');
        visorCtx.beginPath();
      ...
      if (cord.value) {
        ...
        animate(ctx); // ✅ فقط یک بار صدا می‌زنیم
      }
    });
  });
</script>
